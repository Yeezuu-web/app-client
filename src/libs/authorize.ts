...
export const authorize = async ({ context, callback }: AuthorizeProps) => {
  ...
  const { access_token } = store.getState().authReducer // get access_token from memory - redux.
  if (req) {
    // 1. We take cookies (refresh_token) from the client's browser and set it as ours (server-side)
    axios.defaults.headers.common['cookie'] = req.headers.cookie || null as any

    // 1a. If access_token exists assign it as Authorization token in our axios instance.
    ...
    if (!access_token) {
      ...
      try {
        ...
        const responseCookie = setCookie.parse(response.headers['set-cookie'] as any)[0]

        // 3. Set a fresh cookie header for our axios instance.
        axios.defaults.headers.common['cookie'] = cookie.serialize(responseCookie.name, responseCookie.value)
    ...
    try {
      ...
      const cbResponse = await callback(access_token, store, res)
      if (axios.defaults.headers.common['setCookie']) {
        // 2. Optional
        ...
        res.setHeader('set-cookie', axios.defaults.headers.common['setCookie'] as any)

        // 2b. We also update the accessToken
        dispatch(updateAccessToken({ token: (axios.defaults.headers.common['Authorization'] as string).split(' ')[1] }))

        // 2c. Then we clean up the header.
        delete axios.defaults.headers.common['setCookie']
      }
...
export const user = ({ callback }: UserProps) =>
  ...
  wrapper.getServerSideProps(async (context: ContextWithStore | any) => {
